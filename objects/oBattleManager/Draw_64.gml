...
if (oBattleBox.visible == false)
{
	...
	if (showBattleText)
	{
		...
		{
			draw_text_ext(_textX + BUFFER * 3, _textY + BUFFER * 4 + fontSize * a * 2, ds_messages[| a], (fontSize + BUFFER), _textBoxW - BUFFER * 3);
		}
	}
	#endregion
	...
	//PLAYER HEALTH POINT--------------------------
	draw_text(_hpX + 90, _hpY - (5 * BUFFER) + 2, "HP: " + string(global.playerHP) + " / " + string(global.playerMAX_HP) + ";");
	
	//PLAYER NAME--------------------------
	draw_text(_hpX + (2 * BUFFER), _hpY - (5 * BUFFER), string(global.playerName) + ";");
	
	//PLAYER CAGE STATE BAR--------------------------
	var _barCsW = 121;
	var _barCsH = 10;
	draw_text(_hpX + (2 * BUFFER), _hpY, "CS: ");
	draw_sprite(sCSBarBG, 0, _hpX + (BUFFER * 10), _hpY + (BUFFER * 2));
	draw_sprite_stretched(sCSBar, 0, _hpX + (BUFFER * 10), _hpY + (BUFFER * 2), (global.CSvalue/global.CSvalueMax) * _barCsW, _barCsH);
	#endregion
	#region DRAWS ALL THE MONSTER VARIABLES
	var _monsterHpX = guiX; 
	var _monsterHpY = guiY - _textBoxH - (BUFFER * 5) - 4;
	draw_set_color(c_red);
	draw_text(_monsterHpX+ 40, _monsterHpY, "(" + string(global.monsterHP) + " / " + string(global.maxMonsterHp) + ")");
	...
	if(!showBattleText)
	{
		...
		{	
			...
			{
				if (i < 2) { draw_sprite(sArrow, 0, optionX - sprite_get_width(sArrow) + i * 160 - 10, optionY + 2); }
				if (i >= 2) { draw_sprite(sArrow, 0, (optionX + (i - 2) * 160) - sprite_get_width(sArrow) - 10, optionY + 52); }
			}
			
			text = a_text[i];
			
			if (i < 2) { draw_text(optionX + i * 160, optionY, text); }
			if (i >= 2) { draw_text(optionX + (i - 2) * 160, optionY + 50, text); }
		}
	}
	...
	_finalHpWidth = clamp(_finalHpWidth, 0, _hpBarW);
